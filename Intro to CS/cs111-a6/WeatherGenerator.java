...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        // find the location then copy those values into drywet porb and wetwet prob 
        //to find location use for loop to go over each row and check if longitude == drywet[i][0] && latitude == drywet[i][1] then location index is i. The same for wetwet
        //copy the probalitys into the arrays 
        int location = 0;
        ...
        for(int i = 0; i < 12; i++) {
            ...
            drywetProbability[i] = drywet[location][i + 2];
            wetwetProbability[i] = wetwet[location][i + 2];
        }
    }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        double currentDay;
        int currentDayForecast;
        int nextDayForecast;
        int[] forecast = new int[numberOfDays];
        //StdRandom.setSeed(1668136307395L); //set seeed
        
        currentDay = StdRandom.uniform();
        
        currentDayForecast = (currentDay < 0.5) ? WET : DRY;
        forecast[0] = currentDayForecast; //calculate first day of month 

        for(int i = 1; i < numberOfDays; i++) {
            
            currentDay = StdRandom.uniform();
            if (currentDayForecast == WET) {
                if(currentDay <= wetwetProbability) {
                    nextDayForecast = WET;
                }
                else {
                    nextDayForecast = DRY;
                ...
                if(currentDay <= drywetProbability) {
                    nextDayForecast = WET;
                }
                else {
                    nextDayForecast = DRY;
            ...
            forecast[i] = nextDayForecast;
            currentDayForecast = nextDayForecast;
        }
     ...
     *      System.out.println(numberOfWetDryDays(arr, DRY)); //prints out 3
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        // COMPLETE THIS METHOD
        //create count variable to keep track of the count of consecutive modes
        //use a for loop to go over the forecast 
        //if the first index is equal to the mode then add to count until index does not equal mode
        //compare count with max count to update max count
        //reset count to zero 
        int currentCount = 0;
        int maxCount = 0;
        ...
        for(int i = 0; i < forecast.length; i++) {
            ...
            if (forecast[i] == mode) { //keep updating currentCount when forcast equals mode
                currentCount++;

            }
            if(currentCount > maxCount){ //updates max count

                maxCount = currentCount;
            }

            if (forecast[i] != mode) { // resets current count if no longer consecitive matches
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        // COMPLETE THIS METHOD
        //create variables to store bestWeekStartDay and bestWeekCountOfDryDays and variables
        // as we go over the month to calculate for week beginning at day i what are the currentWeekCountofDryDays
        // create a loop that starts at the beginning of the month and stops 7 days before the end of the month
        // for each iteration, count the dry days over the next 7 days and 
        // update bestWeek vars if it is better than current best

        int bestWeekStartDay = 0;
        int bestWeekCountOfDryDays = 0;
        int currentWeekCountofDryDays = 0;

        for(int i = 0; i < forecast.length - 6; i++){ //the -6 means stopping 7 days before. The forcast is the month
            currentWeekCountofDryDays = 0;
            for(int j = i; j < i + 7; j++) {

                if(forecast[j] == DRY) {

                    currentWeekCountofDryDays++;
                }

            }
            //System.out.println("Current  count: " + currentWeekCountofDryDays + " for day " + i);
            if(currentWeekCountofDryDays > bestWeekCountOfDryDays){

                bestWeekCountOfDryDays = currentWeekCountofDryDays;
                bestWeekStartDay = i;
                //System.out.println("Current best count: " + bestWeekCountOfDryDays);
                //System.out.println("best index " + i);


            } 
        }
        return bestWeekStartDay;
    }
