...
public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        //extract the name from description by finding the index of name taking a substring find the index of the semicolon in that substring. 
        String n;
        String temp;  
        temp = description.substring(description.indexOf("name:") + 5, description.length());
        n = temp.substring(0,temp.indexOf(";"));
        //If the type is gas pull find the usage else find usage and charger 
        String type;
        ...
        temp = description.substring(description.indexOf("mile") + 10, description.length());
        usage = temp.substring(0,temp.indexOf(";"));

        String charger = "";
        if(!type.equals("gas")) {

            temp = description.substring(description.indexOf("charger:") + 8, description.length());
        charger = temp.substring(0,temp.indexOf(";"));

        }
        //get paramaters for lease obeject stuff: due, lease length, monthly cost, and mile allownece 
        String due;
          
        temp = description.substring(description.indexOf("due:") + 4, description.length());
        due = temp.substring(0,temp.indexOf(";"));
        ...
        allowance = temp.substring(0,temp.indexOf(";"));


        String monthly;
          
        temp = description.substring(description.indexOf("monthly:") + 8, description.length());
        monthly = temp.substring(0,temp.indexOf(";"));
        
        //create my fuel object 
        double us1 = Double.parseDouble(usage);
        Fuel fuel;
        
        double elec;
        
        if(!type.equals("gas")) {
            elec = Double.parseDouble(charger);
             fuel = new Fuel(us1,elec);
        } else {
            fuel = new Fuel(us1);
        }
        //create my lease object 
        Lease lease = new Lease(Double.parseDouble(due), 
                                Integer.parseInt(length),
                                Double.parseDouble(monthly),
                                Integer.parseInt(allowance));


        //creation of new vehicle 
        Vehicle v = new Vehicle(n,fuel,lease);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        // COMPLETE THIS METHOD
        //for each vehicle calcuate fuel cost, co2, and total cost and set them 
        //call compute c02 
        Lease lease;
        double leaseLength;
        ...
        for(Vehicle v: vehicles) {
             lease = v.getLease();
             leaseLength = (double)lease.getLeaseLength();
            mileageAllowance = lease.getMileageAllowance();
            fuel = v.getFuel();
            usage = fuel.getUsage();

            if(fuel.getType() == Fuel.GAS) {
                co2pu = Fuel.CO2EMITTED_GASCOMBUSTION;
                fuelPrice = gasPrice;
            } else {
                co2pu = Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION;
                fuelPrice = electricityPrice;
            }


            v.setCO2Emission(computeCO2(leaseLength, usage, mileageAllowance, co2pu));
            
            
            v.setFuelCost(computeFuelCost(leaseLength, usage, mileageAllowance, fuelPrice));

            due = lease.getDueAtSigning();
            monthlyCost = lease.getMonthlyCost();
            leaseCost = computeLeaseCost(due, lease.getLeaseLength(), monthlyCost);
        
            totalCost = leaseCost + v.getFuelCost();
            if(fuel.getType() == Fuel.ELECTRIC){ 

                totalCost += fuel.getCharger();
            }
